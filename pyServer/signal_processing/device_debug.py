...
Runs the same flow a real session uses (scan -> select -> open -> start ->
read -> decode), emitting one event per step so the frontend can show the
flow visually. If the start-only sequence yields zero bytes it retries with
the 2.0.5-style gain/mode config burst, DTR/RTS combinations, and alternate
command line endings, because USB-CDC devices sometimes only transmit with
specific control-line states.
...
# The live session (like last-known-good 2.0.4) sends only the start command.
# The Config_GAIN/Oprate burst is what regressed 2.0.5 — tested separately below.
START_ONLY = ["Contl_STRT_AQU"]
FULL_CONFIG = ["Config_GAIN_24", "Oprate_NOR_OPR", "Contl_STRT_AQU"]
READ_SECONDS = 3.0
...
def _attempt(emit: Emit, step_id: str, label: str, com: str, dtr: bool, rts: bool, ending: str,
             commands: list[str] = START_ONLY) -> int:
...
def run_device_debug(emit: Emit) -> None:
    ...
    try:
        ...
        got = _attempt(emit, "standard", "Start-only sequence (as the app does it, like 2.0.4)",
                       com, dtr=True, rts=True, ending="\r\n", commands=START_ONLY)
        if got > 0:
            samples = got // BYTES_PER_SAMPLE
            if samples > 0:
                verdict("ok", "Device streams with the start-only sequence. "
        ...
        if _attempt(emit, "full_config", "Retry with gain/mode config burst (2.0.5 sequence)",
                    com, dtr=True, rts=True, ending="\r\n", commands=FULL_CONFIG) > 0:
            verdict("warn", "Device streams only after the Config_GAIN_24/Oprate_NOR_OPR burst, "
                            "not with start alone - report this.")
            return
